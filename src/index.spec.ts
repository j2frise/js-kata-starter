import { fractionCalc, FractionDTO, ResultOrError } from './index';
...
describe("Testing fractionCalc", () => {
  it("That's a test 1!", function () {
    ...
    const result: ResultOrError = fractionCalc(fraction1Test, fraction2Test);
    const correct = { numerator: 5, denominator: 6 }
    expect(result.response).toEqual(correct);
  });
  ...
  it("That's a test 2!", function () {
    ...
    const result: ResultOrError = fractionCalc(fraction1Test, fraction2Test);
    const correct = { numerator: 2, denominator: 1 }
    expect(result.response).toEqual(correct);
  });
